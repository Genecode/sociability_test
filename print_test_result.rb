class PrintTestResult
  def initialize(test_result)
    @test_results = {
      ...
      "Попробуйте потренировать свою общительность — обращайтесь к людям с различными вопросами, пусть даже" \
      " для начала они будут касаться только работы.",
      (25..29) => "Вы неразговорчивы, предпочитаете одиночество шумным компаниям. Друзей у вас совсем немного. " \
      "Новая работа и новые люди вызывают у вас чувство дискомфорта. Эта особенность характера не ускользнула от" \
      " вашего внимания, и вы часто сердитесь на себя за нее. Вспомните, как легко вы общаетесь, когда чем то очень"\
    ...
    # для отладки
    puts "Баллов #{@test_result}"

    # OPTIMIZE  -  проанализировать кусок на использование штатных методов
